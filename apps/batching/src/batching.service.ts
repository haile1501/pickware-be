...
export class BatchingService {
  ...
  private handleDecisionPointAC() {
    if (this.possibleBatchesNum() == 0) {
      ...
      if (this.waitingThreshold > Date.now()) {
        const waitingPeriod = this.waitingThreshold - Date.now();
        this.isWaiting = true;
        this.logger.log(
  ...
  private possibleBatchesNum(): number {
    ...
    return Math.round(totalWeight / PICKING_DEVICE_CAPACITY);
  }
